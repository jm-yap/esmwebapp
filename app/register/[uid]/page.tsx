...
export default function Form( {params}: Props ) {
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        const response = await AddClient(uid, firstName, lastName, middleName, contactNumber, email);
        if (response) {
    ...
    return (
        <form onSubmit={handleSubmit} className="flex flex-col gap-2 mx-auto max-w-md mt-10">
            ...
            <button className='border border-black rounded-md' type="submit">Update account information</button>
        </form>
    );
}