...
export default function ResponsesPage({ params }: ResponsePageProps) {
  ...
  useEffect(() => {
    const fetchData = () => {
      ...
      getQuestions(params.accessKey, params.surveyID).then((questions: any) => {
        ...
        getResponses(params.accessKey, params.surveyID, questions).then(
          (responses: any) => {
  ...
  let clientNames: clientNameType[] = responses.map((responseObj) => {
    return responseObj.clientName;
  });
      ...
      <main className="main">
        ...
        <div className="surveyInformation">
          <div className="surveyInfoLeft">
            ...
            <div className="surveyInfoLeftBottom">
              <h1>
                <span className="surveyDescHeader">Survey Description</span>
              </h1>
              <h1 className="surveyDesc"> {surveyInfo?.Description}</h1>
              <h1>
                <span className="surveyDescHeader">
                  Required No. of Sessions
                </span>
                <span className="roundedRec">{surveyInfo?.Sessions}</span>
              </h1>
              <h1 style={{ marginBottom: "2%" }}>
                <span className="surveyDescHeader">
                  Minimum Interval (in hours)
                </span>
                <span className="roundedRec">{surveyInfo?.Interval}</span>{" "}
              </h1>

              <Link
                href={`/surveymodule/${params.accessKey}/${params.surveyID}/responses/responsesummary`}
                onClick={handleClick}
              >
                <div
                  style={{
                ...
                >
                  <PeopleAltIcon
                    ...
                    style={{ color: "#E07961" }}
                  />
                  <text className="downloadCSVText">
                    Survey Respondents Status
                  </text>
                </div>
              </Link>
          ...
          <div className="surveyInfoRight">
            ...
            >
              <DownloadIcon
                sx={{ fontSize: 40 }}
                style={{ color: "#E07961", marginBottom: "2%" }}
              />
              {responses.length !== 0 && (
                <CsvDownloader
            ...
            <h1>
              <span className="surveyDescHeader">Anonymous?: </span>
              <span className="roundedRec">{`${moduleAnon ? "Yes" : "No"}`}</span>
            </h1>
            <h1>
              <span className="surveyDescHeader">Opens on: </span>
              <span className="roundedRec">{startDate}</span>
            </h1>
            <h1>
              <span className="surveyDescHeader">Closes on: </span>
              <span className="roundedRec">{endDate}</span>
            </h1>
            <h1>
              <span className="surveyDescHeader">Total Questions: </span>
              <span className="roundedRec">{headerQuestions.length}</span>
            </h1>
          </div>
        </div>
                ...
                {responses.map((response: any) => {
                  return (
                    <tr key={response?.respID}>
                      {(filterName === "None" ||
                        response.clientName === filterName) &&
                        moduleAnon === false && (
                          <>
                      ...
                      {(filterName === "None" ||
                        ...
                        <>
                          ...
                          <td className="tableCell tr:hover">
                            {secondToDateString(response.time)}
                          </td>
                            ...
                            if (perResponse.Response === "") {
                              ...
                              return (
                                <td
                                  ...
                                  className="tableCell tr:hover"
                                >
                                  {(typeof perResponse?.Response === "string" ||
                                    perResponse?.Response instanceof String) &&
                                    perResponse?.Response}
                                  {Array.isArray(perResponse?.Response) &&
                                    perResponse?.Response.map(
                                      (option: any, perResponseIdx: number) => {
                                        if (
                                          perResponseIdx ===
                                          perResponse.Response.length - 1
                                        ) {
                                          return `${option} `;
                                        } else {
                                          return `${option}, `;
        ...
        {responses.length === 0 && isLoading === false && (
          <div className="empty" style={{ marginTop: "100px" }}>
            ...
            />
            <h2>No responses for this survey yet!</h2>
          </div>
        )}
      </main>
    </div>
