...
export default function ResponseSummaryPage({ params }: ResponseSumPageProps) {
  ...
  useEffect(() => {
    const fetchData = () => {
      getAllResponseSummary(params.accessKey, params.surveyID).then(
        (allResponses: any) => {
  ...
  const valueFormatter = (value: number | null) =>
    value == 1 ? `${value} client` : `${value} clients`;
    ...
    <div className="pageProperty">
      <main className="responseSumMain">
        ...
        <div className="aboveSummaryMain">
          <Link
            href={`/surveymodule/${params.accessKey}/${params.surveyID}/responses`}
            onClick={handleClick}
          >
        ...
        <div className="otherStatsMain">
          <div className="otherStatsMainInfo">
            <div style={{ margin: 10 }}>
              <h1>
                <span className="responseSumDescHeader">
                  Number of Participants:{" "}
                </span>
                <span className="responseSumRoundedRec">
                  {summaryArray.length}
                </span>
              </h1>
            </div>
            <div style={{ margin: 10 }}>
              <h1>
                <span className="responseSumDescHeader">Total Sessions: </span>
                <span className="responseSumRoundedRec">
                  {surveyDetails?.Sessions}
                </span>
              </h1>
            </div>
          </div>
          <div className="barChartDiv">
            <h1>
              <span className="responseSumDescHeader">
                Number of Clients per Sessions Answered So Far{" "}
              </span>
            </h1>
            <div
              style={{
                ...
                xAxis={[
                  {
                    scaleType: "band",
                    dataKey: "category",
                    label: "Current Total Sessions Finished",
                  },
                ]}
                series={[
                  {
                      ...
                      series={[
                        {
                          data: [
                            {
                              id: 0,
                              value: summary?.numberOfSessions,
                              color: "#E07961",
